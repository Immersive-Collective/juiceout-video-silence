...
def build_drawtext_base(caption_path: str, font_file: Optional[str]) -> str:
    parts = ["drawtext="]
...
def build_drawtext_alert(alert_path: str, font_file: Optional[str]) -> str:
    # Overlays on top; we keep file blank except during a HIT flash
    parts = ["drawtext="]
    if font_file: parts.append(f"fontfile={esc_drawtext(font_file)}:")
    parts.append(f"textfile={esc_drawtext(alert_path)}:reload=1")
    parts.append(":fontcolor=yellow:fontsize=48")
    parts.append(f":x={LEFT_MARGIN}:y=h-{BOTTOM_MARGIN}")
    parts.append(":box=1:boxcolor=black@0.55:boxborderw=20")
    parts.append(":line_spacing=6")
    parts.append(":fix_bounds=1")
    return "".join(parts)

def build_drawtext_center(center_path: str, font_file: Optional[str]) -> str:
    # Big centered line for exit countdown
    parts = ["drawtext="]
    if font_file: parts.append(f"fontfile={esc_drawtext(font_file)}:")
    parts.append(f"textfile={esc_drawtext(center_path)}:reload=1")
    parts.append(":fontcolor=yellow:fontsize=72")
    parts.append(":x=(w-text_w)/2:y=(h-text_h)/2")
    parts.append(":box=1:boxcolor=black@0.6:boxborderw=30")
...
def start_ffplay_webcam(base_path: str, alert_path: str, center_path: str,
                        font_file: Optional[str], video_idx: int, audio_idx: int,
                        fps: int, size: str):
    global player_proc
    base = build_drawtext_base(base_path, font_file)
    alert = build_drawtext_alert(alert_path, font_file)
    center = build_drawtext_center(center_path, font_file)
    vf = f"{base},{alert},{center},fps=24"
    av_in = f"{video_idx}:{audio_idx}"
    cmd = [
        "ffplay","-hide_banner","-loglevel","warning",
        "-f","avfoundation","-framerate", str(fps), "-video_size", size,
        "-i", av_in, "-threads","2", "-vf", vf, "-bufsize","1024k"
    ]
...
def _join_tail_segments(segments, tail_sec: float) -> str:
    ...
    tail = [s for s in segments if getattr(s,"end",None) is not None and s.end >= cutoff and getattr(s,"text","").strip()]
    if not tail: return ""
...
# ---------- ASR loop ----------
def transcribe_loop(audio_idx: int, base_caption_path: str, force_lang: Optional[str],
                    ...
                    alert_path: str, center_path: str):
    try:
    ...
    # init files
    write_caption(base_caption_path, "")
    write_caption(alert_path, "")
    write_caption(center_path, "")
    logging.info("Caption file: %s", base_caption_path)
    write_line(txt_path, f"# Transcript started {datetime.datetime.now().isoformat(timespec='seconds')}")

    try:
        while True:
            # turn off alert overlay when flash expires
            if alert_until and time.time() > alert_until:
                write_caption(alert_path, "")
                alert_until = 0.0
            ...
            if candidate == "" and onscreen_raw == "":
                ...
                write_caption(base_caption_path, blank)
                last_commit_at = now
                continue
            ...
            # ----- EXIT phrase (centered countdown)
            if exit_regex and exit_regex.search(flat):
                ...
                write_caption(alert_path, "")
                # countdown
                total = max(0, int(round(exit_delay)))
                for n in range(total, -1, -1):
                    msg = f"GOOD BYE — VIDEO WILL CLOSE IN {n}"
                    write_caption(center_path, msg)
                    time.sleep(1.0)
                # one final moment
                time.sleep(0.2)
                break
            ...
            if meaningful:
                write_caption(base_caption_path, wrapped)
                ...
                # keyword flash overlay
                if kw_regex and kw_regex.search(flat):
                    emit_colored_line("HIT", flat, show_vu, ANSI_RED)
                    logging.warning("HIT: %s", flat)
                    write_caption(alert_path, wrapped)      # same text, yellow overlay
                    alert_until = time.time() + HIT_FLASH_SEC
...
def main():
    ...
    ap.add_argument("--kw", default=None, help='Comma-separated keywords to highlight (console red + on-screen yellow flash)')
    ...
    # temp caption files (3 overlays)
    base_caption_path   = os.path.join(tempfile.gettempdir(), "live_caption_webcam.txt")
    alert_caption_path  = os.path.join(tempfile.gettempdir(), "live_caption_alert.txt")
    center_caption_path = os.path.join(tempfile.gettempdir(), "live_caption_center.txt")
    write_caption(base_caption_path, "")
    write_caption(alert_caption_path, "")
    write_caption(center_caption_path, "")

    font_file = pick_font_file()
    start_ffplay_webcam(base_caption_path, alert_caption_path, center_caption_path,
                        font_file, args.video, audio_idx, args.fps, args.size)

    try:
        transcribe_loop(audio_idx, base_caption_path, args.lang, args.model, args.vu,
                        kw_regex, txt_path, srt_path, args.completed_only,
                        exit_regex, args.exit_delay,
                        alert_caption_path, center_caption_path)
    except Exception as e:
    ...
    stop_children()
    # clean temp files
    for p in (base_caption_path, alert_caption_path, center_caption_path):
        try: os.remove(p)
        except: pass
